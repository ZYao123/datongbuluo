...
if($r)
{
	...
	$sql = "select  (SELECT COUNT(id) FROM `#@__usercomment` WHERE `gid` = `#@__goods`.`id` and isshow=1) AS counter ,(SELECT SUM(num)  FROM `#@__goodsshopcart` WHERE `gid` = `#@__goods`.`id` and `#@__goodsshopcart`.norm = `#@__goodsattr`.attrname and `#@__goodsshopcart`.Status='order') AS sumer ,`#@__goods`.`id`,`#@__goods`.`classid`,`#@__goods`.`title`,`#@__goods`.`picurl`,`#@__goodsattr`.`price` 
