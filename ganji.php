...
if($r)
{
	...
	$sql = "select  (SELECT COUNT(id) FROM `#@__usercomment` WHERE `gid` = `#@__goods`.`id` and isshow=1) AS counter ,(SELECT SUM(num)  FROM `#@__goodsshopcart` WHERE `gid` = `#@__goods`.`id` and `#@__goodsshopcart`.norm = `#@__goodsattr`.attrname and `#@__goodsshopcart`.Status='order') AS sumer ,`#@__goods`.`id`,`#@__goods`.`classid`,`#@__goods`.`title`,`#@__goods`.`weight`,`#@__goods`.`retitle`,`#@__goods`.`picurl`,`#@__goodsattr`.`price` 
	...
	$aid!=0 and  $aid=(int)$aid and $sql .=" AND `#@__goodsattribute`.a_id like '%,{$aid},%' ";
	...
	elseif(in_array($ordername,array('sumer','counter','id','price')) and in_array($ordersort,array('asc','desc','id')))
	{
		if ($ordername == 'price')
		{
