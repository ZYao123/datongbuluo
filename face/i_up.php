...
//图片路径
$web['img_up_dir']='../uploads/face';
//源图命名（应用于论坛等程序时可以用会员名编码命名）
$web['img_name_b']='face_'.GetMkTime(date("Y-m-d H:i:s"));
//截图命名
...
//网络图片
  ...
  if ($filename == '' || !preg_match('/^https?:\/\/.+\.(jpg|gif|png)$/i', $filename, $matches)) {
    err('图片格式输入不正确！网址以http://开头！图片格式限(jpg|gif|png)。');
  }
...
window.onload = function() {
  ...
  if(parent!=self){
    if(parent.document.getElementById(\'screenshotsShow\')!=null) {
      parent.document.getElementById(\'screenshotsShow\').innerHTML=\'<img class="i_face_small" src="'.str_replace("../","",$cimg_o).'?'.$t.'" />\';
	  parent.document.getElementById(\'picurl\').value = \''.str_replace("../","",$cimg_o).'\';
	  parent.document.getElementById(\'face\').style.display = "none";
    }
  }
}
</script>';
  ...
  //err('截图成功！<div class="sword">（可点右键另存为）</div><center><a href="'.get_en_url($web['img_up_dir'].'/'.$web['img_name_b']).'.'.$web['img_up_format'].'" target="_blank"><img class="i_face_small" src="'.get_en_url($web['img_up_dir'].'/'.$web['img_name_b']).'.'.$web['img_up_format'].'?'.$t.'" /></a></center>');
  
  err('截图成功！<div class="sword">（可点右键另存为）</div><center><a href="'.$cimg_o.'" target="_blank"><img class="i_face_small" src="'.$cimg_o.'?'.$t.'" /></a></center>');
  exit;
