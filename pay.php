	...
	if($act == 'setpay')
	{
		...
		if(is_array($r))
		{
			...
			if($row['code']=='alipay')
			{
				...
				$file = dirname(__FILE__).'/data/payment/alipay/alipayapi.php';
				if (file_exists($file))
				{
					include_once($file);
			...
			elseif($row['code']=='yinlian')
			{
				...
				$file = dirname(__FILE__).'/data/payment/yinlian/netpayclient_order_submit.php';
				if (file_exists($file))
				{
					include_once($file);
					$smarty->assign('html_text', $html_text); 
					$smarty->display('go_pay_yinlian.html');
				}
				else
				{
					$smarty->display('404.html');
			...
			elseif ($row['code'] == 'wxpay') {
				...
				$file = dirname(__FILE__) . '/data/payment/wxpay/exp/native.php';
				if (file_exists($file)) {
					include_once ($file);
					$smarty -> assign('html_text', $html_text);
					$smarty -> display('go_pay.html');
