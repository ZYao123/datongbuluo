...
include_once('../x.php');	//防注入
header("Content-Type:text/html;charset=utf-8");
if(!defined('IN_LCSHOP'))
{
    exit('Request Error!');
}
define('LCSHOP_INC'		, str_replace('include/config_m.php', '', str_replace('\\', '/', __FILE__)));//路径
define('ADMIN_INC'		, preg_replace("/[\/\\\\]{1,}/", '/', dirname(__FILE__)));//转义字符
define('LCSHOP_DATA'	, LCSHOP_INC . 'data/');//配置文件
define('LCSHOP_TEMP'	, LCSHOP_INC . 'm/templates/');//模版
...
function shopcartnums(){
	...
	if (@$_COOKIE['username'] != ''){
	   $sql1 = "select gid from `#@__goodsshopcart`,`#@__goods`,`#@__goodsattr`  where `#@__goods`.id = `#@__goodsshopcart`.gid AND `#@__goodsattr`.goodsid = `#@__goodsshopcart`.gid  and  `#@__goodsshopcart`.uid = ".$_COOKIE['userid']." AND `#@__goodsattr`.attrname = `#@__goodsshopcart`.norm  and Status = 'cart'";
		...
		$cartnums = $dosql->GetTotalRow(1);
	}else{
		$shopcartnumsa = unserialize(stripslashes($_COOKIE['shop_cart']));
		if($shopcartnumsa!=''){
		   $cartnums = count($shopcartnumsa);	
		}else{
			$cartnums = 0;
		}
...
//echo count(unserialize(stripslashes($_COOKIE['shop_cart'])));
