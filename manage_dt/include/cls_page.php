...
class Page
{
	...
	function GetList()
	{
		...
		else
		{
			...
			$pagetxt = '<span class="tl">每页最多显示条'.$this->pagenum.'跳转到第</span>
		<input type="text" class="pro_text2" name="page" id="gotopage" value="'.$this->page.'"
		onkeyup="this.value=this.value.replace(/[^\d]/g,\'\')" onpaste="return false"
		/>
        <span class="tl"><a href="javascript:;" 
		onclick="gotopage()"
		><img src="templates/default/images/pro_14.jpg" /></a></span><span class="pro_link">';
			...
			$pagetxt .= '</span>共有'.$this->total.'条记录';
		}
	...
	function AjaxGetList()
	{
		...
		else
		{
			...
			if($this->page > 1)
			{
				...
				$pagetxt .= '<a onclick="javascript:getneworders('.$previous.',\'\')" href="javascript:;" title="上一页"><</a>';
			...
			if($this->totalpage < 10)
			{
				for($i=1; $i <= $this->totalpage; $i++)
				{
					...
					{
						$pagetxt .= '<a onclick="javascript:getneworders('.$i.',\'\')" class="num" title="第 '.$i.' 页">'.$i.'</a>';
					}
				}
			}
			else
			{
				...
				for($i=$m; $i<=$b; $i++)
				{
					if($this->page == $i)
					{
						$pagetxt .= '<a onclick="javascript:getneworders('.$i.',\'\'" href="javascript:;"  class="on">'.$i.'</a>';
					}
					else
					{
						$pagetxt .= '<a onclick="javascript:getneworders('.$i.',\'\')" href="javascript:;"  class="num" title="第 '.$i.' 页">'.$i.'</a>';
					}
				}
				if($this->page < $this->totalpage-3)
				{
			...
			if($this->page < $this->totalpage)
			{
				$pagetxt .= '<a onclick="javascript:getneworders('.$next.',\'\')" href="javascript:;"  title="下一页">&gt;</a>';
			}
			else
			{
				$pagetxt .= '<a href="javascript:;" title="已是最后一页">&gt;</a>';
