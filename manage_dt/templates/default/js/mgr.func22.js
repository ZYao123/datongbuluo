...
function AddDmTr(id, str)
{
	...
	if(str==''){
		...
		str += '<span class="delvote"><span class="red">*</span>'+norm_name+'：<input type="hidden" name="norm['+j+'][id]"/><input type="text" name="norm['+j+'][name]" class="v5-input in180" />&nbsp;</span> ';
		str += '<span class="'+colordn+'" name="color[]">'+color+'：<input type="text" name="norm['+j+'][color]" id="color_'+j+'" class="v5-input in180"  onclick="colorpicker(\'colorpanel_'+j+'\', \'color_'+j+'\', \'color_'+j+'\');" /> <span id="colorpanel_'+j+'"></span>&nbsp;</span> ';
		str += '<span class="'+imagedn+'" name="image[]">'+pic+'：<input type="text" name="norm['+j+'][pic]" id="pic_'+i+'" class="v5-input in180" />&nbsp;<a href="javascript:;" onclick="showupload(\'pic_'+i+'\')">'+uploadpic+'</a></span>';
		str += '</td></tr>';
	}
	$("#"+id).append(str);
...
$(function(){  
	$('.tip').mouseover(function(){     
			...
			$('#tip').css({"top":(e.pageY-0)+"px","left":(e.pageX+30)+"px"
		})   
	})
})
