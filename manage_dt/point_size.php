...
include_once(LCSHOP_LANG . 'admin/point_size.php');
...
{
	//验证权限
	chk_privilege("point_site_manage");
	...
	$smarty->display('point_size.html');
}
...
if ($act == 'add')
{
	...
	$smarty->display('point_size_add.html');
}
...
if ($act == 'subadd')
{
	...
	{
		//添加日志
		insert_log($classname, 'add', 'point_size');
		showsucceed($_LANG['php']['add_succeed'], '','point_size.php?act=add&parentid='.$parentid);
		exit();
	}
}
...
if ($act == 'edit')
{
	...
	$smarty->assign("lang"      , $_LANG);
	$smarty->display('point_size_edit.html');
...
if ($act == 'subedit')
{
	...
	{
		//添加日志
		insert_log($classname, 'edit', 'point_size');
		showsucceed($_LANG['php']['add_succeed'], '','point_size.php?act=edit&id='.$id);
		exit();
	}
}
...
if ($act == 'subdel')
{
	...
	foreach ($id as $v)
	{
		...
		//添加日志
		insert_log($classname, 'del', 'point_size');
