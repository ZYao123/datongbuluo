	...
	switch($payResult){
		  ...
		  case "10":
			...
			*/
			...
			$paynum=$bankDealId!=""?$bankDealId:$dealId;
			$result=$dosql->getone("select id from #@__goodsorder where userid='{$userid}' and orderlistnum='{$orderId}' and checkinfo=0"); 
			if($result['id'])
			{
				//修改支付状态为1已支付
				$sql="update #@__goodsorder set  checkinfo=1,paynum='{$paynum}' where userid='{$userid}' and orderlistnum='{$orderId}' 
				and checkinfo=0";
				if($dosql->ExecNoneQuery($sql))
				{
					...
					if($cfg_delhouse==1)//用户设为待发货减库存
					{
						...
						WHERE `gorderlistnum`='{$orderId}' AND `Status`='order'";
