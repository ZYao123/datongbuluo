...
if ($signature_1ocal == $signature)
{
	...
	if ($succ == 'Y')
	{
		...
		**/
		$dosql->ExecNoneQuery("update #@__goodsorder set set checkinfo=1 where UserID=$userid and orderlistnum='$olnum'");
		$rtnUrl="http://".$_SERVER['SERVER_NAME']."/member.php?act=ordercontent&orderid=$orderid";
		echo "<br/><a href=".$rtnUrl.">" . "交易成功返回" . "</a><br/>";
		echo '交易成功';
	}
	else
	{   $rtnUrl="http://".$_SERVER['SERVER_NAME']."/member.php?act=ordercontent&orderid=$orderid";
		echo "<br/><a href=".$rtnUrl.">" . "交易失败返回" . "</a><br/>";
		echo '交易失败！';
		exit;
	}
}
